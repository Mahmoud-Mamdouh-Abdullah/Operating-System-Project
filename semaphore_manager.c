...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int indexOf_semaphore = get_semaphore_object_ID(ownerEnvID,semaphoreName);
	if(indexOf_semaphore == E_SEMAPHORE_NOT_EXISTS)
	{

		struct Semaphore* semaphore_Object = NULL;
		int index_allocation = allocate_semaphore_object(&semaphore_Object);
		if(index_allocation == E_NO_SEMAPHORE)
		{
			return E_NO_SEMAPHORE;
		}
		else
		{
			semaphore_Object->ownerID=ownerEnvID;
			strcpy(semaphore_Object->name,semaphoreName);
			semaphore_Object->value = initialValue;
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	if(indexOf_semaphore != E_SEMAPHORE_NOT_EXISTS)
	{
		semaphores[indexOf_semaphore].value --;
		if(semaphores[indexOf_semaphore].value < 0)
		{
			LIST_INSERT_HEAD(&(semaphores[indexOf_semaphore].env_queue),myenv);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	{
		semaphores[indexOf_semaphore].value ++;
		struct Semaphore semaphore_Object = semaphores[indexOf_semaphore];
		if(semaphore_Object.value <= 0)
		{
			struct Env* myenv = LIST_LAST(&(semaphores[indexOf_semaphore].env_queue));
			LIST_REMOVE(&(semaphores[indexOf_semaphore].env_queue),myenv);
			LIST_INSERT_HEAD(&env_ready_queues[0],myenv);
