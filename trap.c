...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	if(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].empty == 1)
	{
		...
		int aloc_index = allocate_frame(&FrameInfo);

		if(aloc_index != E_NO_MEM)
		{
			map_frame(curenv->env_page_directory,
					FrameInfo,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
	...
	else
	{
		if(isPageReplacmentAlgorithmLRUTimeStamp())
		{
			uint32 *last_ptr = NULL;

			struct Frame_Info* FrameInfo = NULL;
			struct Frame_Info* FrameInfo2 = NULL;


			get_page_table(curenv->env_page_directory,
					(void*)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address,&last_ptr);


			int isUpdated = last_ptr[PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)]&(PERM_MODIFIED);
			...
			if(isUpdated != 0)
			{
				...
				pf_update_env_page(curenv,
						(void*)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address,FrameInfo);

			}
			unmap_frame(curenv->env_page_directory,
					(void*)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address);

			curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address=fault_va;
			//time stamp
			env_page_ws_set_time_stamp(curenv,curenv->page_last_WS_index);

			int aloc_index = allocate_frame(&FrameInfo2);

			if(aloc_index != E_NO_MEM)
			{
				map_frame(curenv->env_page_directory,
						FrameInfo2,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
				int read = pf_read_env_page(curenv,(void*)fault_va);
				if(read == E_PAGE_NOT_EXIST_IN_PF)
				{
					if(fault_va < USTACKTOP && fault_va >= USTACKBOTTOM)
				...
				//Get Minimum time stamp value
			unsigned int MIN_Time = UINT_MAX;
			int Min_index;
			for(int i = 0 ; i < (curenv->page_WS_max_size); i ++)
			{
				uint32 TimeStamp = env_page_ws_get_time_stamp(curenv,i);
			...
			curenv->page_last_WS_index = Min_index;
		}
		else if (isPageReplacmentAlgorithmFIFO())
		{
			...
			int isUpdate = last_ptr[PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)]&(PERM_MODIFIED);

			if(isUpdate != 0)
			{

				FrameInfo2 = get_frame_info(curenv->env_page_directory,
						(void*)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address,&last_ptr);

				pf_update_env_page(curenv,
						(void*)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address,FrameInfo2);
			...
			int index_alocation = allocate_frame(&FrameInfo);

			if(index_alocation != E_NO_MEM)
			{
				map_frame(curenv->env_page_directory,
						FrameInfo,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
		...
		else if (isPageReplacmentAlgorithmCLOCK())
		{
			...
			if((new_ptr[PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)] & PERM_USED) == 0)
			{
				int isUpdate = new_ptr[PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index]
													.virtual_address)] & PERM_MODIFIED;

				if(isUpdate != 0)
				{
					frameInfo = get_frame_info(curenv->env_page_directory,
							(void*)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address,&new_ptr);
					pf_update_env_page(curenv,
										(void*)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address,frameInfo);
				}
				unmap_frame(curenv->env_page_directory,
				...
				curenv->ptr_pageWorkingSet[curenv->page_last_WS_index]
								.virtual_address = VA;


				int index_alocation = allocate_frame(&frameInfo2);
			...
			{
				//cprintf("start while loop");
				// while is used
				while((new_ptr[PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)] & PERM_USED) != 0)
				{
					//make it unused
					new_ptr[PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)] =
							new_ptr[PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)] & (~PERM_USED);
					if(curenv->page_last_WS_index >= curenv->page_WS_max_size - 1)
						curenv->page_last_WS_index = 0;
					else
						curenv->page_last_WS_index ++;
